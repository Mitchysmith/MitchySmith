...
function initWork() {
  ...
  if (!window.state.work.onboardees)  window.state.work.onboardees  = [];
}
...
function renderWork() {
  ...
  el.innerHTML = `
    ...
    <div class="work-top-tabs">
      ...
      <button class="work-top-tab ${workTopTab==='onboarding'?'active':''}" data-tab="onboarding">🚀 Onboarding</button>
    </div>
    ...
    <div class="work-top-panel ${workTopTab==='onboarding'?'active':''}" id="wtp-onboarding"></div>
...
function buildTasksPanel() {
  ...
  p.innerHTML = `
    ...
    <div class="work-widgets-row">
      ${['longterm', 'panel', 'personal'].map(key => `
...
function buildOnboardingPanel() {
  const p = document.getElementById('wtp-onboarding');
  if (!p) return;

  p.innerHTML = `
    <div class="ob-add-card">
      <div class="ob-add-title">Add New Onboarding</div>
      <div class="ob-add-row">
        <input type="text" id="ob-new-id"    class="work-text-input" placeholder="ID — e.g. EMP-042" style="max-width:160px;flex-shrink:0" />
        <input type="text" id="ob-new-title" class="work-text-input" placeholder="Full name / title" />
        <button class="btn btn-primary" onclick="addOnboarding()">Add</button>
      </div>
    </div>
    <div id="ob-list"></div>`;

  document.getElementById('ob-new-title')?.addEventListener('keydown', e => {
    if (e.key === 'Enter') addOnboarding();
  });
...
function addOnboarding() {
  const idEl    = document.getElementById('ob-new-id');
  const titleEl = document.getElementById('ob-new-title');
  const title   = titleEl?.value.trim();
  if (!title) return;

  window.state.work.onboardees.unshift({
    uid:            'ob_' + Date.now(),
    id:             idEl?.value.trim() || '—',
    title,
    comment:        '',
...
function toggleOnboardingDone(uid) {
  const rec = (window.state.work.onboardees || []).find(r => r.uid === uid);
  if (!rec) return;
  rec.done = !rec.done;
  saveState();
  refreshOnboardingList();
  ...
  if (rec.done) {
    massConfetti();
    setTimeout(massConfetti, 650);
    const overlay = document.createElement('div');
    overlay.className = 'daily-complete-overlay';
    overlay.innerHTML = `
      <div class="daily-complete-card">
        <div class="daily-complete-emoji">🎉</div>
        <div class="daily-complete-title">Onboarding Complete!</div>
        <div class="daily-complete-sub">${rec.title} is fully onboarded. Great work getting them set up.</div>
...
function saveOnboardingComment(uid) {
  const rec   = (window.state.work.onboardees || []).find(r => r.uid === uid);
  const input = document.getElementById(`ob-comment-${uid}`);
  if (!rec || !input) return;
  const text = input.value.trim();
  if (!text) return;

  // Push to audit trail and update visible status
  rec.commentHistory.push({ text, ts: Date.now() });
  rec.comment = text;
...
function deleteOnboarding(uid) {
  window.state.work.onboardees = (window.state.work.onboardees || []).filter(r => r.uid !== uid);
  saveState();
  refreshOnboardingList();
}
...
function refreshOnboardingList() {
  ...
  if (!el) return;
  const records = window.state.work.onboardees || [];

  if (!records.length) {
    el.innerHTML = `<div class="task-list-empty" style="margin-top:16px">No onboardings yet — add one above.</div>`;
    return;
  }
  ...
  let html = '';
  if (pending.length) html += pending.map(r => obCardHtml(r)).join('');
  if (done.length) {
    html += `<div class="ob-done-hdr">✓ Completed (${done.length})</div>`;
    html += done.map(r => obCardHtml(r)).join('');
  }
  el.innerHTML = html;

  records.filter(r => !r.done).forEach(r => {
    const inp = document.getElementById(`ob-comment-${r.uid}`);
    if (inp) inp.addEventListener('keydown', e => {
      if (e.key === 'Enter') saveOnboardingComment(r.uid);
    });
  });
}

function obCardHtml(r) {
  const isDone = r.done;
  const histCount = r.commentHistory?.length || 0;
  return `
    <div class="ob-card ${isDone ? 'ob-card-done' : ''}">
      <div class="ob-card-top">
        <span class="ob-id-badge">${r.id}</span>
        <span class="ob-title-text">${r.title}</span>
        ${histCount ? `<span class="ob-hist-pill" title="${histCount} note(s) on record">${histCount} note${histCount !== 1 ? 's' : ''}</span>` : ''}
        <label class="ob-done-label" title="${isDone ? 'Mark incomplete' : 'Mark fully onboarded'}">
          <input type="checkbox" class="ob-done-chk-input" ${isDone ? 'checked' : ''}
            onchange="toggleOnboardingDone('${r.uid}')" />
          <span class="ob-done-tick-box"></span>
          <span class="ob-done-txt">${isDone ? '✓ Onboarded' : 'In progress'}</span>
        </label>
        <button class="ob-delete-btn" onclick="deleteOnboarding('${r.uid}')" title="Remove record">✕</button>
      </div>

      ${r.comment ? `
        <div class="ob-status-row">
          <span class="ob-status-lbl">Status</span>
          <span class="ob-status-text">${r.comment}</span>
        </div>` : ''}

      ${!isDone ? `
        <div class="ob-comment-row">
          <input type="text" id="ob-comment-${r.uid}" class="work-text-input ob-comment-input"
            placeholder="Add a status note…" />
          <div class="ob-tick-wrap" onclick="saveOnboardingComment('${r.uid}')" title="Tick to archive this note">
            <span class="ob-tick-box-ui"></span>
            <span class="ob-tick-lbl">Save &amp; archive note</span>
          </div>
        </div>` : ''}
    </div>`;
}
...
function _runDayPlan() {
  const allTasks  = [];
  const listOrder = { daily: 0, panel: 1, longterm: 2, personal: 3 };
  const priOrder  = { high: 0, med: 1, low: 2 };
